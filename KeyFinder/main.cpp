...
typedef struct {
    ...
    bool random = false;
    bool resumed = false;
    bool randomWindows = false;

    // XOR filter integration
    std::string xorfltFile = "";
    bool xorfltOnly = false;
}RunConfig;
...
// Global XOR filter reader instance (loaded if --xorflt is provided)
static XorFilter::Reader g_xorReader;
...
void resultCallback(KeySearchResult info)
{
	if(_config.resultsFile.length() != 0) {
		...
		// Append toggled flag for clarity (0 = original prefix, 1 = flipped prefix)
		s += std::string(" toggled=") + (info.toggled ? "1" : "0");
	...
	if(info.compressed) {
		logStr += std::string("Hit mode:    ") + (info.toggled ? "flipped prefix (02<->03)" : "original prefix") + "\n";
	}
...
void usage()
{
    printf("KeyFinder OPTIONS [TARGETS]\n");
    ...
    printf("-r, --random            Randomize starting offsets (non-sequential scan)\n");
    printf("--random-windows        Repeatedly pick a random window of ~24e9 keys (400MKey/s * 60s) over full keyspace, scan it, then pick another\n");
    ...
    // XOR filter options
    printf("--xorflt FILE           Load XOR filter (.xorflt) built by xorfilterbuilder\n");
    printf("--xorflt-only           Use XOR filter only (no explicit targets). Note: device-side integration may be required.\n");
}
...
static std::string toHex(const unsigned char *buf, int len)
{
...
int run()
{
    ...
    try {
        ...
        if(!_config.randomWindows) {
            ...
            // Apply random mode before device initialization; ignore when continuing from checkpoint
            bool enableRandom = _config.random && !_config.resumed;
            d->setRandom(enableRandom);
            ...
            // Allow XOR-filter-only mode without explicit targets
            f.setIgnoreEmptyTargets(_config.xorfltOnly);

            f.init();

#ifdef BUILD_CUDA
            {
                CudaKeySearchDevice *cdev = dynamic_cast<CudaKeySearchDevice*>(d);
                if(cdev != NULL && g_xorReader.isLoaded()) {
                    cdev->enableXorFilter(g_xorReader.fp().data(), (unsigned int)g_xorReader.arrlen(), g_xorReader.mask(), (unsigned long long)g_xorReader.seed());
                    Logger::log(LogLevel::Info, "Device-side XOR filter enabled");
                }
            }
#endif

            // Only upload explicit targets when provided, or when not in XOR-only mode
            bool haveExplicitTargetsFirst = (!_config.targetsFile.empty()) || (!_config.targets.empty());
            if(!_config.xorfltOnly || haveExplicitTargetsFirst) {
                ...
                Logger::log(LogLevel::Info, "XOR-only: skipping explicit target upload to device");
            }
            ...
            // Random windows mode: keep a single device alive and rebase between windows
            const uint64_t kWindowCount = 400000000ULL * 60ULL; // 400 MKey/s * 60s

            KeySearchDevice *d = getDeviceContext(_devices[_config.device], _config.blocks, _config.threads, _config.pointsPerThread);
            // Keep deterministic ordering within each window
            d->setRandom(false);

            bool firstWindow = true;

            while(true) {
                // If resuming from a checkpoint, finish the current window first
                if(!_config.resumed) {
                    // Compute window span in terms of key values considering stride
                    secp256k1::uint256 windowSpan = secp256k1::uint256((uint64_t)kWindowCount) * _config.stride;

                    // Ensure the window fits: start in [1, N-1-windowSpan]
                    secp256k1::uint256 one(1);
                    secp256k1::uint256 maxStart = secp256k1::N - one - windowSpan;

                    crypto::Rng rng;
                    unsigned char buf[32];
                    secp256k1::uint256 r;

                    // Rejection sampling to get uniform r in [0, maxStart]
                    do {
                        rng.get(buf, 32);
                        std::string hex = toHex(buf, 32);
                        r = secp256k1::uint256(hex);
                    } while(r.cmp(maxStart) > 0);

                    _config.startKey = r + one;
                    _config.nextKey = _config.startKey;
                    _config.endKey = _config.startKey + windowSpan;
                }

                Logger::log(LogLevel::Info, "[RandomWindow] Compression: " + getCompressionString(_config.compression));
                Logger::log(LogLevel::Info, "[RandomWindow] Starting at: " + _config.nextKey.toString());
                Logger::log(LogLevel::Info, "[RandomWindow] Ending at:   " + _config.endKey.toString());
                Logger::log(LogLevel::Info, "[RandomWindow] Counting by: " + _config.stride.toString());
                ...
                if(firstWindow) {
                    ...
                    // Allow XOR-filter-only mode without explicit targets for this window
                    f.setIgnoreEmptyTargets(_config.xorfltOnly);

                    // First window: initialize device
                    f.init();
                    ...
                    // Only upload explicit targets when provided, or when not in XOR-only mode
                    bool haveExplicitTargets = (!_config.targetsFile.empty()) || (!_config.targets.empty());
                    if(!_config.xorfltOnly || haveExplicitTargets) {
                        if(!_config.targetsFile.empty()) {
                            f.setTargets(_config.targetsFile);
                        } else {
                            f.setTargets(_config.targets);
                    ...
                    // Subsequent windows: rebase device state instead of reinitializing
                    secp256k1::uint256 currentNext = d->getNextKey();
                    secp256k1::uint256 deltaPriv = secp256k1::subModN(_config.nextKey, currentNext);

                    #ifdef BUILD_CUDA
                    if(deltaPriv.isZero() == false) {
                        CudaKeySearchDevice *cdev = dynamic_cast<CudaKeySearchDevice*>(d);
                        if(cdev != NULL) {
                            cdev->rebaseTo(deltaPriv);
                        } else {
                            // Fallback: if not CUDA device, break to re-create device as before
                            Logger::log(LogLevel::Info, "Rebase not supported on this device type; recreating context.");
                            delete d;
                            d = getDeviceContext(_devices[_config.device], _config.blocks, _config.threads, _config.pointsPerThread);
                            d->setRandom(false);
                        }
                    }
                    #endif
                    ...
                    // Allow XOR-filter-only mode without explicit targets for subsequent windows
                    f.setIgnoreEmptyTargets(_config.xorfltOnly);

                    // Important: do NOT call f.init() here to avoid regenerating starting points
                    // Only upload explicit targets when provided, or when not in XOR-only mode
                    bool haveExplicitTargets = (!_config.targetsFile.empty()) || (!_config.targets.empty());
                    if(!_config.xorfltOnly || haveExplicitTargets) {
                        if(!_config.targetsFile.empty()) {
                ...
                // After finishing a window (either by reaching end or no targets), clear resumed flag
                _config.resumed = false;

                // Continue to next random window automatically
            }
        }
...
int main(int argc, char **argv)
{
    ...
    parser.add("", "--random-windows", false);
    ...
    // XOR filter options
    parser.add("", "--xorflt", true);
    parser.add("", "--xorflt-only", false);
	...
	for(unsigned int i = 0; i < args.size(); i++) {
		...
		try {
			if(optArg.equals("-t", "--threads")) {
                ...
                _config.random = true;
            } else if(optArg.equals("", "--random-windows")) {
                _config.randomWindows = true;
            } else if(optArg.equals("", "--xorflt")) {
                _config.xorfltFile = optArg.arg;
            } else if(optArg.equals("", "--xorflt-only")) {
                _config.xorfltOnly = true;
            }
    ...
    // Load XOR filter if requested
    if(!_config.xorfltFile.empty()) {
        if(!g_xorReader.load(_config.xorfltFile)) {
            // Error already logged in reader
            return 1;
        }
    }

    // If --xorflt-only is set, ensure we have a loaded XOR filter
    if(_config.xorfltOnly) {
        if(_config.xorfltFile.empty()) {
            Logger::log(LogLevel::Error, "--xorflt-only requires --xorflt <filter_file>");
            return 1;
        }
    }
    ...
    if(ops.size() == 0) {
        if(_config.targetsFile.length() == 0) {
            if(_config.xorfltOnly) {
                // In XOR-filter-only mode, do NOT fallback to default addresses.txt even if present
                Logger::log(LogLevel::Info, "--xorflt-only: running with device-side XOR filter only (no explicit targets)");
            } else {
                // Try default 'addresses.txt' in current working directory
                std::ifstream defaultFile("addresses.txt");
                if(defaultFile.good()) {
                    _config.targetsFile = "addresses.txt";
                    Logger::log(LogLevel::Info, "No targets specified. Using default 'addresses.txt'");
                } else {
