...
// New: constants for rebase delta point
__constant__ unsigned int _REBASE_X[8];
__constant__ unsigned int _REBASE_Y[8];
...
// New: set the delta point for rebase
cudaError_t setRebasePoint(const secp256k1::uint256 &x, const secp256k1::uint256 &y)
{
    ...
    cudaError_t err = cudaMemcpyToSymbol(_REBASE_X, xWords, sizeof(unsigned int) * 8);
    if(err) {
        return err;
    }

    return cudaMemcpyToSymbol(_REBASE_Y, yWords, sizeof(unsigned int) * 8);
}
...
__device__ void setResultFound(int idx, bool compressed, bool toggled, unsigned int x[8], unsigned int y[8], unsigned int digest[5])
{
    CudaDeviceResult r;
...
__device__ void doIteration(int pointsPerThread, int compression)
{
    ...
    for(int i = 0; i < pointsPerThread; i++) {
        ...
        if(compression == PointCompressionType::COMPRESSED || compression == PointCompressionType::BOTH) {
            // first try with natural parity
            unsigned int yParity = readIntLSW(yPtr, i);
            hashPublicKeyCompressed(x, yParity, digest);
            ...
            if(checkHash(digest)) {
                ...
                // try toggled (flipped) parity
                unsigned int y[8];
                readInt(yPtr, i, y);
                unsigned int digest2[5];
                hashPublicKeyCompressed(x, yParity ^ 1u, digest2);
                if(checkHash(digest2)) {
                    setResultFound(i, true, true, x, y, digest2);
                }
            }
        }
...
// New: device-side rebase iteration (no hashing)
__device__ void doRebase(int pointsPerThread)
{
    unsigned int *chain = _CHAIN[0];
    unsigned int *xPtr = ec::getXPtr();
    unsigned int *yPtr = ec::getYPtr();
    ...
    for(int i = 0; i < pointsPerThread; i++) {
        unsigned int x[8];
        readInt(xPtr, i, x);
        // Build batch for adding uniform delta point to each entry
        beginBatchAdd(_REBASE_X, x, chain, i, i, inverse);
    }
    ...
    for(int i = pointsPerThread - 1; i >= 0; i--) {
        unsigned int newX[8];
        unsigned int newY[8];
        completeBatchAdd(_REBASE_X, _REBASE_Y, xPtr, yPtr, i, i, chain, inverse, newX, newY);
        writeInt(xPtr, i, newX);
        writeInt(yPtr, i, newY);
    }
}
