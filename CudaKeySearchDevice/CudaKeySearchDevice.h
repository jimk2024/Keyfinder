...
struct CudaDeviceResult {
    ...
    int idx;
    bool compressed;
    bool toggled; // true if match was found using flipped (03<->02) compressed prefix
    unsigned int x[8];
    unsigned int y[8];
...
private:
    ...
    bool verifyKey(const secp256k1::uint256 &privateKey, const secp256k1::ecpoint &publicKey, const unsigned int hash[5], bool compressed, bool toggled);
    bool _random = false;

    // Track whether device-side XOR filter is active
    bool _xorEnabled = false;
...
public:
    ...
    // Enable/disable randomized starting offsets
    virtual void setRandom(bool randomMode) { _random = randomMode; }

    // New: rebase current device keyset by adding deltaPriv*G to all cached public keys
    void rebaseTo(const secp256k1::uint256 &deltaPriv);

    // Enable XOR filter on device; throws on CUDA error
    void enableXorFilter(const unsigned short *fp, unsigned int arrlen, unsigned short mask, unsigned long long seed) {
        cudaCall(_targetLookup.enableXorFilter(fp, arrlen, mask, seed));
        _xorEnabled = true;
    }
};
