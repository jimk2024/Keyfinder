...
// XOR filter device constants
__constant__ unsigned short *_XOR_FP[1];
__constant__ unsigned int _XOR_ARRLEN[1];
__constant__ unsigned short _XOR_MASK[1];
__constant__ unsigned long long _XOR_SEED[1];
...
cudaError_t CudaHashLookup::setTargetConstantMemory(const std::vector<struct hash160> &targets)
{
	...
	// 修正：_USE_BLOOM_FILTER 是 unsigned int 常量，必须使用 sizeof(unsigned int)
...
void CudaHashLookup::initializeBloomFilter64(const std::vector<struct hash160> &targets, unsigned int *filter, unsigned long long mask)
{
	for(unsigned int k = 0; k < targets.size(); k++) {
		...
		for(int i = 0; i < 5; i++) {
			unsigned int f = filter[idx[i] / 32];

			filter[idx[i] / 32] = f | (0x01 << (idx[i] % 32));
		}
	}
}
...
cudaError_t CudaHashLookup::setTargetBloomFilter(const std::vector<struct hash160> &targets)
{
	...
	if(bloomFilterBits <= 32) {
		// 修正：_BLOOM_FILTER_MASK 是 unsigned int 常量，必须使用 sizeof(unsigned int)
		...
		// 修正：_BLOOM_FILTER_MASK64 是 unsigned long long 常量，必须使用 sizeof(unsigned long long)
...
// XOR filter hashing must match XorFilter::Reader
__device__ __forceinline__ unsigned long long mix64_dev(unsigned long long x) {
    x ^= x >> 30; x *= 0xbf58476d1ce4e5b9ULL;
    x ^= x >> 27; x *= 0x94d049bb133111ebULL;
    x ^= x >> 31; return x;
}
__device__ __forceinline__ unsigned long long hashKey64_dev(const unsigned int h[5], unsigned long long seed) {
    unsigned long long x = seed ^ 0x9e3779b97f4a7c15ULL;
    x = mix64_dev(x ^ ((((unsigned long long)h[0]) << 32) | h[1]));
    x = mix64_dev(x ^ ((((unsigned long long)h[2]) << 32) | h[3]));
    x = mix64_dev(x ^ ((((unsigned long long)h[4]) << 32) | (h[0] ^ h[2])));
    return x;
}

__device__ bool checkXorFilter(const unsigned int hash[5]) {
    unsigned short *fp = _XOR_FP[0];
    if(fp == NULL) return false;
    unsigned int m = _XOR_ARRLEN[0];
    unsigned long long seed = _XOR_SEED[0];
    unsigned short mask = _XOR_MASK[0];

    unsigned long long base = hashKey64_dev(hash, seed);
    unsigned long long a = mix64_dev(base + 0x9e3779b97f4a7c15ULL);
    unsigned long long b = mix64_dev(base + 0x3c6ef372fe94f82aULL);
    unsigned long long c = mix64_dev(base + 0xdaa66d2c7ddef7bULL);
    unsigned int i0 = (unsigned int)(a % m);
    unsigned int i1 = (unsigned int)(b % m);
    unsigned int i2 = (unsigned int)(c % m);
    unsigned short f = fp[i0] ^ fp[i1] ^ fp[i2];

    unsigned long long t = mix64_dev(hashKey64_dev(hash, seed) ^ 0xD6E8FEB86659FD93ULL);
    unsigned short want = (unsigned short)(t & mask);
    return (f & mask) == want;
}
...
__device__ bool checkHash(const unsigned int hash[5])
{
	...
	} else if(_XOR_FP[0] != NULL) {
        return checkXorFilter(hash);
	} else {
...
// Host-side: upload XOR filter
cudaError_t CudaHashLookup::setXorFilter(const unsigned short *fp, unsigned int arrlen, unsigned short mask, unsigned long long seed)
{
    // Free previous
    if(_xorFpPtr != NULL) {
        cudaFree(_xorFpPtr);
    ...
    size_t bytes = sizeof(unsigned short) * (size_t)arrlen;
    cudaError_t err = cudaMalloc(&_xorFpPtr, bytes);
    if(err) return err;
    err = cudaMemcpy(_xorFpPtr, fp, bytes, cudaMemcpyHostToDevice);
    if(err) return err;

    err = cudaMemcpyToSymbol(_XOR_FP, &_xorFpPtr, sizeof(unsigned short*));
    if(err) return err;
    err = cudaMemcpyToSymbol(_XOR_ARRLEN, &_xorArrLen, sizeof(unsigned int));
    if(err) return err;
    err = cudaMemcpyToSymbol(_XOR_MASK, &_xorMask, sizeof(unsigned short));
    if(err) return err;
    err = cudaMemcpyToSymbol(_XOR_SEED, &_xorSeed, sizeof(unsigned long long));
    if(err) return err;

    // Disable bloom/constants
    unsigned int zero = 0;
    err = cudaMemcpyToSymbol(_USE_BLOOM_FILTER, &zero, sizeof(unsigned int));
    if(err) return err;

    return cudaSuccess;
}

cudaError_t CudaHashLookup::enableXorFilter(const unsigned short *fp, unsigned int arrlen, unsigned short mask, unsigned long long seed)
{
    return setXorFilter(fp, arrlen, mask, seed);
}