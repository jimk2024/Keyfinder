...
private:
	...
	// When true, do not stop run() when _targets becomes empty (XOR-filter-only mode)
	bool _ignoreEmptyTargets = false;
...
public:
	...
	// Enable/disable ignoring empty targets (useful for XOR-filter-only mode)
	inline void setIgnoreEmptyTargets(bool v) { _ignoreEmptyTargets = v; }
};
