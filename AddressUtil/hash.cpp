...
void Hash::hashPublicKeyCompressed(const unsigned int *x, const unsigned int *y, unsigned int *digest)
{
	...
	// Use original parity: odd -> 0x03, even -> 0x02
	if(y[7] & 0x01) {
...
// New function: hash with flipped compressed prefix
void Hash::hashPublicKeyCompressedFlipped(const unsigned int *x, const unsigned int *y, unsigned int *digest)
{
	unsigned int msg[16] = { 0 };
	unsigned int sha256Digest[8];
	...
	// Flip the parity: odd -> 0x02, even -> 0x03
	if(y[7] & 0x01) {
		msg[0] = (x[0] >> 8) | 0x02000000;
	} else {
		msg[0] = (x[0] >> 8) | 0x03000000;
